...
{% set languageSheet = {
  ...
  messageHeader: "Hola {{firstName}} {{lastName}},",
  messageBody: "Conforme a lo solicitado durante tu reciente visita a {{ecommerceName}}, confirmamos que tu cuenta de usuario {{userEmail}} ha sido eliminada de nuestra base de datos.",
  messageSignature: "Atentamente, el equipo de {{ecommerceName}}"
} %}
<tr>
  <td style="padding: 20px 20px 20px 20px; color: #000; font-family: Arial, Helvetica, sans-serif; font-size: 18px; line-height: 20px; text-align: center;">
            ...
            <div style="padding: 0 0;">
              <table style="border-bottom: 2px solid #dcdcdc; color: #454545; font-family: Arial, Helvetica, sans-serif; font-size: 14px; vertical-align: middle;" width="88%" cellpadding="0" cellspacing="0" align="center">
                <tbody>
                  ...
                  {% for product in recommend.products %}
                    <tr>
                      <td style="text-align: left; font-family: Arial, Helvetica, sans-serif; font-size: 14px; border-bottom: 1px solid #dcdcdc;">
                        <table style="color: #454545; font-family: Arial, Helvetica, sans-serif; font-size: 14px; vertical-align: middle;" >
                          <tr>
                            <td style="padding: 10px 0px;">
                              <img width="60" src="{{ product.images.smallImage }}" alt="{{ product.name }}" onerror="this.style.display='none';">
                            </td>
                            <td style="padding: 10px 10px;">
                                <a href="{{ product.productLink }}" title="{{ product.name }}" target="_blank" rel="noreferrer" style="text-decoration: none;">
                                  {% if product.sku|length %} <span style="color: #454545; text-decoration: none;">{{ product.sku }} - </span> {% endif %}
                                  <span style="color: #454545; text-decoration: none;">{{ product.name }}</span>
                                </a>
                            </td>
                          </tr>
                        </table>
                      </td>
                      <td style="text-align: right; padding-right: 8px; border-bottom: 1px solid #dcdcdc;"><span><span>{{ product.price }}</span></span></td>
                    </tr>
                  {% endfor %}
...
{% set languageSheet = {
  ...
  messageBody2: "Haz clic <a target='_blank' href='{{url}}' style='color: #000;'>aquí</a> si deseas recuperar tu pedido.",
  messageSignature: "Atentamente, el equipo de {{ecommerceName}}"
} %}
<tr>
  <td style="padding: 20px 20px 20px 20px; color: #000; font-family: Arial, Helvetica, sans-serif; font-size: 18px; line-height: 20px; text-align: center;">
...
<tr>
  <td style="padding: 0px 0px;background-color: #fff;" align="center">
    ...
    46: {
      name: `Recomendación de lista de compra`,
      subject: `{{ general.ecommerceName }} - {{ recommend.name }} te recomienda una lista de productos`,
      html: `
{% set languageSheet = {
  premessage: "Recomendación de lista de productos",
  messageHeader1: "Tu amigo/a {{name}} ({{email}}) te envía esta lista de productos que puede resultarte de interés.",
  messageBody: "Si necesitas más información acerca de estos productos puedes contactar con nosotros en <a href='{{ecommerceUrl}}' style='color: #000;'>{{ecommerceName}}</a>",
  messageSignature: "Atentamente, el equipo de {{ecommerceName}}",
  recommendedProducts: "Productos recomendados",
  product: "Producto",
  price: "Precio",
